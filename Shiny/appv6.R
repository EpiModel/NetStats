...
# artnet<-readRDS("H:/ART-NET/artnet4shiny.rda")
# artnetLong4shiny<-readRDS("H:/ART-NET/artnetlong4shiny.rda")
# artnetLong<-readRDS("H:/ART-NET/ARTNet-long.rda")
# rawdata<-read_sav("H:/ART-NET/ART-Net_Raw.sav")
...
#RACE
#artnetLong$partracesmatch <- rep(NA, nrow(artnetLong))
artnetLong$partracesmatch[which(artnetLong$race.cat == "black" &
                             artnetLong$partracecat == "black")] <- "Black - Black"
artnetLong$partracesmatch[which(artnetLong$race.cat == "white" &
                                  artnetLong$partracecat == "white")] <- "White - White"
artnetLong$partracesmatch[which(artnetLong$race.cat == "hispanic" &
                                  artnetLong$partracecat == "hispanic")] <- "Hispanic - Hispanic"
artnetLong$partracesmatch[which(artnetLong$race.cat == "other" &
...
ui <- fluidPage(
  ...
  navbarPage("ART-Net",
             ...
             tabPanel("Partnership Duration Mixing (T4)",
                      sidebarLayout(
                        # Input(s)
                        sidebarPanel(
                          titlePanel(h3("Inputs", align="center")),
             ...
             tabPanel("Partnership Mixing (T5)",
                      sidebarLayout(
                        # Input(s)
                        sidebarPanel(
                          titlePanel(h3("Inputs", align="center")),
                          selectInput(inputId ="x5",
                                      label = "Stratifications",
                                      choices = c(
                                        "Race"="partraces",
...
#DATA INPUTS-----------------------------
  ...
  #   switch(input$y3,
  #          "Duration"=artnetLong$p_duration
  #   )
  # })
  ...
  datasetInputx4<-reactive({
    switch(input$x3,
           "Age Category"=artnetLong$age.cat,
           "HIV Status"=artnetLong$hiv,
           "Race Category"= artnetLong$partracesmatch
    )
  })
...
#TABLE 1----------------------------------------
  ...
  output$table1<-renderTable({rbind(table1a(),c("Total", round(colSums(table1a()[,2:3],0))))})
...
#TABLE 3----------------------------------------------
  ...
  table3alls<-reactive(
    artnetLong %>%
      group_by_(input$x3) %>%
      filter((p_ONGOING == 1 & p_duration < 2150) & (ptype==1 | ptype==2) & (p_RAI==1 | p_IAI==1 | p_ROI==1 | p_IOI==1)) %>%
      summarise(n=n(),
                mean(p_duration,na.rm = TRUE),
                sd(p_duration,na.rm=TRUE),
                median(p_duration,na.rm = TRUE)
      ) %>%
      setNames(c(" ", "n", "Mean", "SD", "Median"))
    )
  ...
  table3a<-reactive(
    artnetLong %>%
      group_by_(input$x3) %>%
      filter((p_ONGOING == 1 & p_duration < 2150) & (ptype==1) & (p_RAI==1 | p_IAI==1 | p_ROI==1 | p_IOI==1)) %>%
      summarise(n=n(),
                mean(p_duration,na.rm = TRUE),
                sd(p_duration,na.rm=TRUE),
                median(p_duration,na.rm = TRUE)
  ...
  table3b<-reactive(
    artnetLong %>%
      group_by_(input$x3) %>%
      filter((p_ONGOING == 1 & p_duration < 2150) & (ptype==2) & (p_RAI==1 | p_IAI==1 | p_ROI==1 | p_IOI==1))%>%
      summarise(n=n(),
                mean(p_duration,na.rm = TRUE),
                sd(p_duration,na.rm=TRUE),
                median(p_duration,na.rm = TRUE)
...
#TABLE 5------------------------------------------------------------
  ...
  table5allpart <- reactive({
    if (input$x5=="edgeagediff" | input$x5=="sqrtedgeagediff")
    {
      artnetLong4shiny %>%
        summarise_at(vars(input$x5),
                     funs(n=n(),
                          mean(.,na.rm = TRUE),
                          sd(.,na.rm=TRUE),
                          median(.,na.rm = TRUE)
  ...
  #Bind Table Rows Tab 5
  output$table5all<-reactive({
  if(input$x5=="edgeagediff" | input$x5=="sqrtedgeagediff")
  {
    renderTable({rbind(table5allpart(),table5allparttotal())})
  }
  else
  {
    renderTable({rbind(table5allpart(),c("Total", round(colSums(table5allpart()[,2:3],0))))})
  }
  })

  #output$table5all<-renderTable({rbind(table5allpart(),c("Total", round(colSums(table5allpart()[,2:3],2))))})
  output$table5main<-renderTable({rbind(table5mains(),table5maintotal())})
  output$table5casual<-renderTable({rbind(table5casuals(),table5casualtotal())})
  output$table5oneoff<-renderTable({rbind(table5oneoffs(),table5oneofftotal())})
...
#CREATE DENSITY PLOT TAB 3-Every
  ...
  output$densityplot3all<-renderPlot({
    ggplot(data=t3every, aes_string(x=t3every$p_duration, group=input$x3, color=input$x3))+
      geom_density(size=1) + theme_bw() + labs(y="Density",
                                               x= "Duration",
...
#FILTERED DENSITY PLOT TAB 3- MAIN
  ...
  output$densityplot3main<-renderPlot({
    ggplot(data=t3main, aes_string(x=t3main$p_duration, group=input$x3, color=input$x3))+
      geom_density(size=1) + theme_bw()+ labs(y="Density",
                                              x="Duration",
                                              color=if(input$x3=="age.cat"){print ("Age Category")}
                                              else if(input$x3=="race.cat"){print ("Race Category")}
...
#CREATE DENSITY PLOT TAB 3-Casual
  ...
  output$densityplot3casual<-renderPlot({
    ggplot(data=t3casual, aes_string(x=t3casual$p_duration, group=input$x3, color=input$x3))+
      geom_density(size=1) + theme_bw()+ labs(y="Density",
                                              x="Duration",
                                              color=if(input$x3=="age.cat"){print ("Age Category")}
                                              else if(input$x3=="race.cat"){print ("Race Category")}
...
#CREATE DENSITY PLOT T4
t4every<-
  artnetLong %>%
  filter(ptype==1 | ptype==2 & p_RAI==1 & p_IAI==1 & p_ROI==1 & p_IOI==1)
output$densityplot4all<-renderPlot({
  ggplot(data=t4every, aes_string(x=t4every$p_duration, group=input$x4, color=input$x4))+
    geom_density(size=1) + theme_bw() + labs(y="Density",
                                             x= "Duration",
                                             color=if(input$x4=="age.cat"){print ("Age Category")}
                                             else if(input$x4=="race.cat"){print ("Race Category")}
                                             else if(input$x4=="hiv"){print ("HIV Status")}
                                             )
})
...
output$densityplot4main<-renderPlot({
  ggplot(data=t4main, aes_string(x=t4main$p_duration, group=input$x4, color=input$x4))+
    geom_density(size=1) + theme_bw()+ labs(y="Density",
                                            x="Duration",
                                            color=if(input$x4=="age.cat"){print ("Age Category")}
                                            else if(input$x4=="race.cat"){print ("Race Category")}
...
output$densityplot4casual<-renderPlot({
  ggplot(data=t4casual, aes_string(x=t4casual$p_duration, group=input$x4, color=input$x4))+
    geom_density(size=1) + theme_bw()+ labs(y="Density",
                                            x="Duration",
                                            color=if(input$x4=="age.cat"){print ("Age Category")}
                                            else if(input$x4=="race.cat"){print ("Race Category")}
