...
shinyserver<-(function(input, output) {
  ...
  output$summarystatsall <- renderTable({
    if (input$x4=="")
    {
      summarytab<- 
        estimatesbysite %>%
        ...
        summarise(
          median(adj_rate_hosp,na.rm = TRUE),
          range=max(adj_rate_hosp,na.rm= TRUE) - min(adj_rate_hosp,na.rm = TRUE),
          q1=quantile(adj_rate_hosp,probs=.25,na.rm=TRUE),
          q3=quantile(adj_rate_hosp,probs=.75,na.rm=TRUE),
          IQR(adj_rate_hosp,na.rm = TRUE)
        ) %>%
        setNames(c(" ", "Median Rate by Age Group","Range", "Q1", "Q3", "IQR"))
    } 
    else if (inputx4="")
    {
      summarytab<-
  ...
  # table4alls<-reactive(
  #   artnetLong4shiny %>%
  #     if (input$x4== edgeagediff)
  #       #group_by_(input$x4==edgeagediff) %>%
  #       summarise(
  #         mean(na.rm = TRUE),
  #         sd(na.rm=TRUE),
  #         median(na.rm=TRUE)
  ...
  output$table4all<-renderTable({rbind(table4alls())})
