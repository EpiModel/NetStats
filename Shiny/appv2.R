...
ui <- fluidPage(
  ...
  navbarPage("ART-NET Data",
             ...
             tabPanel("Partnership Degree (T2)",
                      sidebarLayout(
                        ...
                        mainPanel(
                          ...
                          fluidRow(
                            column(4,
                                   h4("Total Degree- One-Off Partnerships", align="center"),
...
shinyserver<-(function(input, output) {
  ...
  datasetInputy2<- reactive({
    switch(input$y2,
           ...
           "Number of One-Off Partners"=artnet$rate.oo.aioi.part
    )
  })
...
#TABLE 4------------------------------------------------------------
  ...
  output$table4all<-renderTable(table4allpart())
...
#PLOTS----------------------------------------------  
  ...
  output$densityplot2all<-renderPlot({
    ggplot(data=t2every, aes_string(x=input$y2, group=input$x2, color=input$x2))+ stat_density(geom="line") +
